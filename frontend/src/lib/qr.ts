 ...
 *
 * Hand-rolled rather than pulled from a package so the bundle carries no QR dependency for what
 * is a few hundred lines of finite-field arithmetic. Lived inside the UPI collection screen
 * until the profile card needed the same encoder; it is pure data in, matrix out, so it belongs
